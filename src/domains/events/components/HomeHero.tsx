...
/**
 * El hero de Inicio en todos sus estados (ver buildHomeHeroState): festival
 * en curso, show hoy, cuenta regresiva, la mañana después, sólo pasado,
 * primera vez y sin sesión. Los cuatro últimos viven en HomeHeroStates.tsx.
  ...
  return (
    <section className="relative min-h-screen snap-start overflow-hidden bg-ritual-panel">
      ...
      */}
      ...
      {/* En mobile el aire de abajo es el de la pila fija (talón + acción):
          sin él el nombre del show queda tapado. */}
      <div className="relative z-20 flex flex-col justify-end min-h-screen px-6 md:px-10 pb-[var(--ritual-mobile-clearance)] md:pb-16 pt-24">
        {days === 0 && (
        ...
        <div className="flex flex-wrap items-center gap-3 mt-8">
          <button
            type="button"
            onClick={openTicket}
            className="hidden md:inline-block ritual-cta font-figure text-xl tracking-wider bg-ritual-red text-ritual-bone px-8 py-3.5"
          >
            ABRIR MI ENTRADA
          </button>
          <Link
            href={routes.events.detail(event.id)}
            className="ritual-btn inline-flex items-center min-h-[44px] font-label text-[10px] tracking-[0.14em] text-ritual-gray-text uppercase border border-ritual-border-2 px-4 py-3"
          >
            Ver función
          </Link>
        </div>
      ...
      {/* Sin esto el scroll snap se lee como que la pantalla termina acá. En
          mobile no hace falta: ahí abajo ya está la pila fija. */}
      <div className="pointer-events-none absolute inset-x-0 bottom-6 z-20 hidden md:flex flex-col items-center gap-1.5 ritual-bob">
