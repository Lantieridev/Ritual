  ...
  return (
    <form onSubmit={handleSubmit} className="max-w-xl space-y-6">
      ...
      <FormField label="Nombre del recital" id="name" required>
        <input id="name" name="name" type="text" required defaultValue={event?.name ?? ''} placeholder="Ej: Coldplay en Movistar Arena" className={inputClass} />
      </FormField>
      ...
      <FormField label="Sede" id="venue_id" required hint={venues.length === 0 ? 'No hay sedes cargadas. Agregá una sede desde el listado de sedes.' : undefined}>
        <select id="venue_id" name="venue_id" required defaultValue={event?.venue_id ?? ''} className={inputClass}>
      ...
      <FormField label="Artistas en el lineup" id="lineup-artists" hint={artists.length === 0 ? 'No hay artistas cargados. Agregá artistas desde el listado para poder armar lineups.' : undefined}>
        {artists.length === 0 ? <span id="lineup-artists" className="text-zinc-500" /> : (
          <div className="max-h-48 overflow-y-auto rounded-lg border border-white/20 bg-white/5 p-3 space-y-2" role="group" aria-label="Seleccionar artistas">
            {artists.map((a) => (
              <label key={a.id} className="flex items-center gap-2 cursor-pointer text-sm text-white hover:bg-white/5 rounded px-2 py-1">
                <input type="checkbox" checked={selectedArtistIds.has(a.id)} onChange={() => toggleArtist(a.id)} className="rounded border-white/30 bg-white/5 text-white focus:ring-white/30" />
                <span>{a.name}</span>
