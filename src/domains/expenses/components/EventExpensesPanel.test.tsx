...
describe('EventExpensesPanel', () => {
  ...
  it('quick-add: toggling it in, adding an expense inserts it, updates the total and groups, and expands its category', async () => {
    vi.mocked(insertExpense).mockResolvedValue({ id: 'x4' })
    renderPanel()
    ...
    await waitFor(() => {
      expect(insertExpense).toHaveBeenCalledWith(
        expect.objectContaining({ amount: 2000, category: 'Merch', event_id: 'ev-1', date: '2026-05-01' })
      )
    })
  ...
  it('inline edit: editing an expense updates it in place without any redirect', async () => {
    ...
    await waitFor(() => {
      expect(modifyExpense).toHaveBeenCalledWith('x3', { amount: 16000, category: 'Entrada', note: '' })
    })
  ...
  it('inline delete: confirming removal takes the expense out of its group and out of the total', async () => {
    ...
    await waitFor(() => {
      expect(removeExpense).toHaveBeenCalledWith('x3')
    })
  ...
  it('inline delete: a failed removal keeps the expense and shows the error', async () => {
    vi.mocked(removeExpense).mockResolvedValue({ error: 'No se pudo eliminar.' })
    renderPanel()
