...
import { SignOutButton, ProfileHubHeader, ProfileHubGrid } from '@/src/domains/auth/components'
...
import { buildProfileHub } from '@/src/domains/auth/profile-hub-view'
...
export default async function ProfilePage() {
    ...
    const [profile, myEvents, stats, expensesSummary] = await Promise.all([
        findProfile(user.id),
        listMyEvents(),
        getStats(),
        summarizeExpenses(user.id),
    ])
    ...
    const hub = buildProfileHub({
        ...
        showsAttended: wentCount,
        uniqueArtists: stats.uniqueArtists,
        yearShows: stats.showsByYear[String(currentYear)] ?? 0,
        yearSpend: expensesSummary.byYear[String(currentYear)] ?? 0,
        year: currentYear,
    })

    return (
        <PageShell title="Mi Perfil" action={<span className="hidden md:flex"><SignOutButton /></span>}>
            {/* Mobile: hub "Vos" (perfil-mobile) — header con avatar/monograma,
                sheet "Tu cuenta" y grid 2x2 de atajos. Reemplaza el carnet de
                escritorio con la vista compacta de la spec; el logout de acá
                vive en el sheet, por eso el SignOutButton de arriba se oculta
                en mobile (D-6) en vez de mostrarse dos veces. */}
            <div className="md:hidden" data-testid="perfil-mobile">
                <ProfileHubHeader
                    eyebrow={hub.eyebrow}
                    displayName={hub.displayName}
                    monogram={hub.monogram}
                    avatarUrl={hub.avatarUrl}
                    statsLine={hub.statsLine}
                />
                <ProfileHubGrid cells={hub.cells} />
            </div>

            {/* Escritorio: carnet de socio + cuenta, sin cambios de este WU
                (Desktop Non-Regression). */}
            <div className="hidden md:block" data-testid="perfil-desktop">
